...
#define SPECIAL_FRAMES_UP 6
#define SPECIAL_FRAMES_TIME_MOVE 10
#define SPECIAL_FRAMES_FADE 6
#define SPECIAL_FRAMES_DOWN 6

// Box padding
#define BOX_SIDE_PADDING 4
#define BOX_BOTTOM_PADDING 4

// Fade colors for digit transition
static const GColor FADE_COLORS[] = {DARK_BLUE,
                                     GColorBlueMoon,
                                     GColorPictonBlue,
                                     GColorVividCerulean,
                                     GColorTiffanyBlue,
...
static void prv_end_special_animation(TextLayer *grid_layers[])
{
    ...
    grid_motion_reset_offsets_at_indices(TIME_POSITIONS, 4);
...
{
    const int side_padding = BOX_SIDE_PADDING;
    const int bottom_padding = BOX_BOTTOM_PADDING;
...
void minute_transition_update_animation(TextLayer *grid_layers[], const GRect grid_orig[30])
{
    ...
    {
        s_special_frame++;
        int total = SPECIAL_FRAMES_TIME_MOVE;
    ...
    else if (s_special_state == 3)
    {
        ...
        if (s_special_frame >= total)
        {
            ...
            for (int j = 0; j < 4; j++)
            {
                ...
                layer_set_hidden(text_layer_get_layer(grid_layers[index]), false);
                text_layer_set_text_color(grid_layers[index], FADE_COLORS[0]);
            }
        }
    }
    else if (s_special_state == 4)
    {
        ...
        if (step > SPECIAL_FRAMES_FADE)
            step = SPECIAL_FRAMES_FADE;
        int color_index = (step * (int)(sizeof(FADE_COLORS) / sizeof(FADE_COLORS[0]) - 1)) / SPECIAL_FRAMES_FADE;
        for (int j = 0; j < 4; j++)
        {
            int index = TIME_POSITIONS[j];
            text_layer_set_text_color(grid_layers[index], FADE_COLORS[color_index]);
        }
        if (s_special_frame >= SPECIAL_FRAMES_FADE)
        {
